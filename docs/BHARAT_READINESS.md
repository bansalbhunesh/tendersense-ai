...
| OCR engine selection | **Working (Tesseract + pdfplumber by default; optional PaddleOCR via `requirements-ocr.txt`)** | See `ai-service/app/ocr_pipeline.py`. PaddleOCR ships Devanagari-trained weights when installed; Tesseract uses `hin` + `eng` traineddata when present. |
